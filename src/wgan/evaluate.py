...
def evaluate_wgan_stats(dataloader_real, device, cfg):
    """
    Evaluate a trained WGAN-GP model using FID, KID, KS-statistic, and EMD.
    ...
    generator = Generator256(z_dim=cfg['model']['z_dim'], ngf=64).to(device)
    state_dict = torch.load(os.path.join(ckpt_dir, cfg['output']['best_model_name']), map_location=device)
    ...
    with torch.no_grad():
        for _ in range(num_batches):
            z = torch.randn(batch_size, cfg['model']['z_dim'], 1, 1, device=device)
